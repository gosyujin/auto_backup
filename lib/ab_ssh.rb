...
module AB
  module SSH
    ...
    def transfer(file, setting)
      ...
      # リモートにsetting.ymlに記述したREMOTE_DIRが存在するかチェックするスクリプト
      # 存在する場合は exist しない場合は notexist を返す
      exist_bash = "if [ -e #{remote_dir} ]; then echo exist; else echo notexist; fi"
      exist_remote_dir = exec_ssh(:ssh, exist_bash, setting)
